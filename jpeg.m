...
record jpeginforec =
    ...
    int framebytes          !width*height*ncomponents
...
proc main=
    ...
    p:=loadjpegm(file, width, height, ncomp)
...
!width/height info is returned via references in the parameter list
    ...
    width:=hdr.width
    height:=hdr.height
    ncomp:=hdr.ncomponents

    return p
end

export proc freejpegm(ref byte p)=
    free(p)
end
