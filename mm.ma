...
!global func writeallpcl:ref strbuffer= nil end
!global func writepst:ref strbuffer=nil end
!global func strpclstr(pcl p, int buffsize)ichar="" end
...
export proc pc_setsymmode(psymbol d, int m)=
	d.mode:=m
end
...
!CPL "ILOAD", PCLNAMES[NEWPC.OPCODE]
...
!CAN SLOW THINGS DOWN
			elsif pc.opndtype=int_opnd and newpc.opcode=kaddpx and 
				pcnext.opcode in [kiloadx, kistorex] then
				newpc.opcode:=pcnext.opcode
				newpc.mode:=pcnext.mode
				newpc.size:=pc.size
				newpc.extra+:=pc.value*pcnext.scale+pcnext.extra
				++pc
...
!
	...
	if p.opcode=kjumpret then
		mccodex.regfreed[r0]:=0
	end
end
			...
			if not d.reg then
				...
				if ispfloat(d.mode) then
					if d.reg>=xr4 then				!not in-situ param
						genmc(m_movq, mgenxreg(d.reg), mgenxreg(regoffset+xr0, psize[d.mode]))
					fi
...
!CPL "AND/TEST",++NN,"//", isreg0(m.a), m.a=m2.a, m2.a=m2.b, =endr0(m3),
!	MSTROPND(M.A), MSTROPND(M2.A), MSTROPND(M2.B)

!				if isreg0(m.a) and m.a=m2.a=m2.b and endr0(m3) then
!THIS VERSION CAN SLOW THINGS DOWN
				if isreg0(m.a) and isreg0(m2.a) and isreg0(m2.b) and endr0(m3) then
!CPL "-------->DEL TEST",++MM
					m:=deletemcl(m2)
				fi
			fi
...
!CAN SLOW THINGS DOWN
		when m_lea then
			case m2.opcode
			when m_mov then
				if isreg0(m.a) and isreg0(m2.b) and isreg(m2.a) and endr0(m2) then
!					CPL "LEA/MOV R0"
					m.a:=m2.a
					m:=deletemcl(m2)
				elsif isreg1(m.a) and isreg1(m2.b) and isreg(m2.a) and endr1(m2) then
!					CPL "LEA/MOV R1*************"
					m.a:=m2.a
					m:=deletemcl(m2)
				end
...
func isreg1(mclopnd a)int=
	if not a then return 0 fi
	if a.mode=a_reg and a.reg=r1 then return 1 fi
	return 0
end
...
func endr1(ref mclrec m)int=
	return m.regfreed[r1]
end
...
global func getdispname(psymbol d)ichar=
	...
	static []ichar specialnames= (
		"eq",
		"ne")

	for i to specialnames.len do
		if eqstring(d.name, specialnames[i]) then
			strcpy(str, "$")
			strcat(str, d.name)
			return str
		end
...
!global func runprogram(ref librec lib, int cmdskip=0)int=
...
!PC_COMMENT("INDEX/ILOADNEXT/block")
...
!	wxstr((passlevel=dll_pass|"importdll "|"importlib "))
	wxstr("ffi (")
!	wxstr("importlib ")
	wxstr(extractbasefile(modulename))
...
!	wxstr("    ")
!	wxstr("    ")
	wxstr((d.mode=tvoid|"ffi proc "|"ffi func "))
	wxstr(d.name)
	wxstr("(")
...
!CPL "TOP1",P.NAME
!CPL =POWNER.SUBPROGNO, SUBPROGS[POWNER.SUBPROGNO].NAME
!CPL =SUBPROGNO, SUBPROGS[SUBPROGNO].NAME
!CPL =SCOPENAMES[P.SCOPE]
!CPL =P.ISIMPORT
				if powner.subprogno=subprogno or		!within same subprog
					 p.scope in [program_scope, export_scope] or	!any func exported from
!					 p.scope=export_scope or
					 p.isimport then 				!visible outside subprog
					++extcount			!if an ext match is closest, there can only be one
!!CPL "TOP2",P.NAME
!IF P.SCOPE=PROGRAM_SCOPE THEN
!CPL "///PROG SCOPE", P.NAME
!END
					extdef:=p
...
!need to create suitable holding typename in advance
	...
	if kwd=kfunctionsym then
		if lx.symbol=lbracksym then		!possible params
			...
			if lx.symbol=colonsym or lx.symbol=sendtosym then
				lex()
				nretvalues:=readreturntype(stproc,retmodes,1)
			elsif typestarterset[lx.symbol] or lx.symbol=namesym then
				nretvalues:=readreturntype(stproc,retmodes,1)
			fi
		elsif lx.symbol=colonsym or lx.symbol=sendtosym then
...
func readreturntype(symbol owner, []int &retmodes, int isref=0)int=
...
global enumdata [0:]ichar scopenames=
	(Module_scope=0,	"Module"), ! 		!intra-module/module
	(subprog_scope,		"Subprog"), ! 		!inter-module/subprog
	(program_scope,		"Program"), ! 		!inter-suprog/program
	(export_scope,		"Export"), ! 		!inter-program/cross-program
end
...
!check whether p is const expr
	...
	when jname then
		...
		if p.def.nameid=dllprocid then return fi
...
proc tx_notl(unit p,a)=
	tpass(a)
	txerror("Void") when a.mode=tvoid
	p.mode:=tbool
end
...
!module mnoos
module mwindllc
=== msyswini.m 0 1 45/57 ===
module msys
module mlib
module mclib
module mwindows
module mwindllc
=== msyslinc.m 0 1 46/57 ===
module msysc
module mlib
module mclib
module mlinux
...
26 mm_genpcl.m 0 0
27 mm_libpcl.m 0 0
28 mm_blockpcl.m 0 0
29 mm_assem.m 0 0
30 mm_assemaux.m 0 0
31 mm_decls.m 0 0
32 mm_diags.m 0 0
33 mm_exportq.m 0 0
34 mm_lex.m 0 0
35 mm_lib.m 0 0
36 mm_libsources.m 0 0
37 mm_modules.m 0 0
38 mm_name.m 0 0
39 mm_parse.m 0 0
40 mm_support.m 0 0
